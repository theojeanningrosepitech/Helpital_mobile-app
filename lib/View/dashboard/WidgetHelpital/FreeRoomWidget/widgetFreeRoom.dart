...
class WidgetFreeRoom extends StatefulWidget {
  const WidgetFreeRoom({Key key}) : super(key: key);

  @override
  _WidgetFreeRoomState createState() => _WidgetFreeRoomState();
}

class _WidgetFreeRoomState extends State<WidgetFreeRoom> {
  List<Room> listRoom;
  _WidgetFreeRoomState() {
    RoomsService().getRoomsListWithPatient().then((value) {
      setState(() {
        listRoom = value;
      });
    });
  ...
  void setFreeRoom() {
    ...
    listRoom.forEach((element) {
      ...
      if (element.type.id == 2 && element.patients.length <= 1) {
